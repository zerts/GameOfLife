//
// Created by zerts on 19.10.16.
//

#ifndef GAMEOFLIFE_FASTPLAYER_H
#define GAMEOFLIFE_FASTPLAYER_H
...
void workeromp(size_t turns, size_t width,
...
#pragma omp parallel num_threads(threads) shared(field, progress)
    {
        ...
        //workeromp(turns, width, field, progress);
    }
    /*for (size_t i = 0; i < threads; i++) {
        workers.push_back(thread(worker, i, turns, i * width, min(size, (i + 1u) * width),
                                 ref(field), ref(progress)));
    }
    for (size_t i = 0; i < threads; i++) {
        workers[i].join();
    }*/
